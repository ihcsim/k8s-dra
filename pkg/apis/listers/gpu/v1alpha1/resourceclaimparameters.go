...
type ResourceClaimParametersNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ResourceClaimParameters, error)
...
// Get retrieves the ResourceClaimParameters from the indexer for a given namespace and name.
func (s resourceClaimParametersNamespaceLister) Get(name string) (*v1alpha1.ResourceClaimParameters, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("resourceclaimparameters"), name)
	}
	return obj.(*v1alpha1.ResourceClaimParameters), nil
}
